...
variable "aws_cidr_block_vartf" {
  ...
  validation {
    condition     = contains(["10.0.0.0/16", "192.168.10.0/16"], var.aws_cidr_block_vartf) # Here we are defining the condition for the variable, so that it will validate the cidr block as per the condition
...
variable "aws_vpc_name" {
  ...
  validation {
    condition     = length(var.aws_vpc_name) > 10       # Here we are defining the condition for the variable, so that it will validate the vpc name as per the condition
